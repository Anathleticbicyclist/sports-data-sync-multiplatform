...
class AutoSyncWorker(
    ...
    override suspend fun doWork(): Result {
        ...
        return try {
            // v8.2.3: 任务级自动同步统一调度——存在启用的 autoSync 任务时优先按任务执行（多来源×多目标），否则回退全局批量
            ...
            if (autoTasks.isNotEmpty()) {
                ...
                try {
                    updateForeground("任务同步完成: 新上传$totalSynced 条", source, targets.first(), intervalMin)
                    val nm = applicationContext.getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager
                    ...
                    // v8.2.3.7: 详细大文本——任务行 + 成功/失败/跳过明细（限量防止通知过长）
                    val detail = buildString {
                        ...
                        if (skippedDetails.isNotEmpty()) {
                            append("⏭️ 跳过 ${skippedDetails.size} 条（已在同步记忆）\n")
                        }
                        append("间隔${intervalMin}分钟 | 下次约${intervalMin}分钟后")
                    }
                    val taskNames = autoTasks.joinToString("、") { it.name }
                    val notification = NotificationCompat.Builder(applicationContext, CHANNEL_ID)
                        .setContentTitle(if (hasFail) "鸡翅幸哲迈进OB 任务同步（部分失败）" else "鸡翅幸哲迈进OB 任务同步")
    ...
    private suspend fun downloadActivity(source: DataSource, cred: String, record: ActivityRecord): ByteArray? {
        return when (source) {
            ...
            DataSource.ZWIFT -> {
                // v8.3.2: Zwift 403 修复——S3 直链失败自动回退 API 鉴权下载
                try {
                    val zw = zwiftApi.downloadFit(record.extra ?: "", cred, record.id)
                    plog("✅ Zwift 下载成功 (v8.3.3: S3直链+详情预检+API兜底)")
                    zw
                } catch (e: Exception) {
                    plog("❌ Zwift 下载失败: ${e.message}")
                    null
                }
            }
    ...
    /** v7.8.3: Wahoo作为自动同步源——刷新token后拉取活动列表（与手动同步一致） */

...
### v8.6.6 (2026-09-26)
- **[修复]** 修复同步bug，UI优化响应，提升体验
- **[新功能]** 新建任务/运行任务纳入体验计划：体验未激活/过期时不可新建、不可执行；取消15天免费体验，全新安装须申请体验码，覆盖安装有效期不受影响
- **[新功能]** 首页任务区体验状态横幅（未激活/过期显示，点击可申请/输入体验码）
- **[优化]** 自动同步任务受体验计划约束，过期自动跳过并提示更新体验码
...
- **[优化]** Outbase登录卡固定到自动检测卡片下方，独占整行
- **[修复]** Outbase卡片上传下载统计不刷新
...
- 设置页新增佳明间同步健康数据（步数/睡眠/HRV/压力）开关
- 修复IGP注销后自动登录旧账号的问题
- 修复运行任务时底部导航栏偶尔消失的问题
...
### v8.4.0 (2026-09-19)
- 实验室新增 Polar Flow / 悦跑圈 / Zepp Life 接入（登录后在实验室使用，实测上传下载双通后再进首页）
- 两步路登录误判修复：登录前清 cookie 打开登录页，未登录不再误判已登录

### v8.3.5 (2026-09-19)
- 同步健康数据（步数/睡眠/HRV/压力）佳明 CN↔COM 双向
- 引入游标增量、查重预检，同步更快更准

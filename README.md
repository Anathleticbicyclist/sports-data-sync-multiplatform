...
[![Version](https://img.shields.io/badge/Version-v8.3.5-brightgreen)]()
...
| **健康数据同步** | v8.3.5 新建任务开启「同步健康数据」后，佳明（CN↔COM）双向同步步数/睡眠/HRV/压力，按天下载 wellness → 逐条上传，409 重复自动跳过、419 未绑定健康设备自动停止提示 |
| **游标增量** | v8.3.5 按源平台最后同步时间增量，失败不推进游标、下次自动重试 |
| **查重预检** | v8.3.5 上传前预拉目标佳明列表，按「开始时间\|时长」键跳过已存在记录（不下载不上传） |
...
- **🧪 健康数据同步（v8.3.5 亮点）**：佳明中国↔佳明国际双向同步步数/睡眠/HRV/压力等健康数据（Wellness），按天自动下载源端健康数据（zip→FIT）并逐条串行上传目标端，500ms 间隔防佳明风控；409 重复自动跳过、419 目标未绑定健康设备自动停止并提示
- **🚀 游标增量 + 查重预检（v8.3.5）**：增量同步升级为「按源平台最后同步时间游标」——失败不推进游标、下次自动重试，不再依赖万条 ID 记忆；上传前预拉目标佳明列表，按「开始时间\|时长」键预判重复，命中即不下载不上传，同步更快更准
...
### v8.3.5 (2026-09-19)
- 🧪 **同步健康数据（步数/睡眠/HRV/压力）佳明（CN↔COM 双向）**：新建任务开启「同步健康数据」后，自动下载源佳明最近 7 天 Wellness 数据（zip→FIT）逐条上传目标佳明；409 重复自动跳过、419 目标未绑定健康设备立即停止并提示
- 🚀 **游标增量 + 查重预检**：增量升级为「按源平台最后同步时间游标」（失败不推进、下次自动重试）；上传前预拉目标佳明列表按「开始时间|时长」键预判重复，命中即不下载不上传，同步更快更准

### v8.3.4 (2026-09-18)
- 🧩 优化轨迹合并优化贴纸体验：导出与预览像素级对齐（数据/单位间隙、字体位置修复）
- 🔐 优化佳明国际国内通道登陆逻辑：密码错误/两步验证/风控限流任一通道即停并弹窗引导检查（避免污染全部通道触发24H风控），可强制忽略警告继续登录；国内国际失败弹窗风格一致（含滑动日志窗口、复制日志、多吃两口排查引导）
- 🛠️ Zwift 下载权限预检：试用订阅账号无 FIT 下载权限时给出明确提示（fitnessData=NOT_AVAILABLE）
- 🔒 开源仓库合规化：继续剔除不开源模块代码

### v8.3.3 (2026-09-18)
- 🧩 贴纸3D轨迹对齐Outbase：细橙线+窗帘式竖向投影（任何角度灰带竖直向下）、无发光无平面线、4000点全量高精度渲染
- 🛠️ Zwift下载修复：S3 key编码修复 + 下载前权限预检 + 列表分页上限50
- 🐛 修复导出轨迹偏移（导出与预览完全一致）
- 🔒 开源仓库合规化：移除不开源模块代码

### v8.3.2 (2026-09-18)
- 🧩 贴纸3D立体投影重制：粗缎带立体轨道、海拔Z轴抬升、强垂直投影、受光/背光层次
- 🐛 修复导出轨迹偏移（导出与预览完全一致）
- 🛠️ Zwift下载403修复（S3直链失败自动回退API鉴权下载）

### v8.3.1 (2026-09-18)
- 🧩 贴纸3D轨迹立体投射：轨迹按海拔Z轴立体抬升、可左右/俯仰拖拽旋转、正下方垂直投影、随高度明暗渐变
- ✨ 平面/3D一键切换，轨迹永不截断
- 📱 实验室页支持滑动浏览
...
- 修复 Keep 导出轨迹为空：Keep 轨迹字段兼容多结构解析，时间换算修复，导出的 FIT 不再单点兜底
- 贴纸导出发光优化：光晕轮廓由粗实线改为柔光（与预览一致，颜色更鲜艳真实）
- 编辑区按钮风格统一：连续多选修复（可正常选区间、起点有橙色标记）、填充看不清的按钮改为浅底深字、编辑/完成/导出按钮统一描边样式

### v8.1.2 (2026-09-16)
- 贴纸导出与预览完全一致：轨迹颜色更鲜艳真实（橙→蓝渐变完整还原），照片不再变形压缩
- 照片卡片圆角更精致、周边柔和阴影过渡更自然，照片加载更稳定
- 轨迹粗细升级为 6 档可调（最细→最粗），调节立即生效
